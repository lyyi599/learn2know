...
```
params = 4 x (hidden_size x input_size +   # W matrices
       hidden_size x hidden_size +   # U matrices
       hidden_size)          # b vectors

    = 4 x hidden_size x (input_size + hidden_size + 1)
```

### Example (input=32, hidden=64):
```
params = 4 x 64 x (32 + 64 + 1)
    = 4 x 64 x 97
    = 24,832 parameters
```
...
|-----------|--------|-------|--------|
| `W_f, W_i, W_c, W_o` | Xavier | U(-sqrt(6/(in+out)), sqrt(6/(in+out))) | Maintain activation variance |
...
| Issue | Solution |
|-------|----------|
| Vanishing gradients | OK Orthogonal initialization for U matrices |
| Exploding gradients | OK Gradient clipping (not implemented) |
| Can't learn long dependencies | OK Forget bias = 1.0 |
| Unstable training | OK Xavier initialization for W matrices |
| NaN in forward pass | OK Numerically stable sigmoid |
