...
def demonstrate_solvability():
  ...
  for i in range(3):
    ...
    if found:
      print(f"  OK Pair found in sequence!")
    else:
      print(f"  Pairs: {pairs}")
  ...
  for i in range(3):
    ...
    print(f"  OK Logical answer generated")
...
def create_difficulty_visualization():
  ...
  plt.tight_layout()
  difficulty_path = IMAGES_DIR / "task_difficulty_scaling.png"
...
def visualize_multiple_examples():
  ...
  for idx in [1, 2, 3]:
    ...
    tracking_path = IMAGES_DIR / f"tracking_ex{idx}.png"
    plt.savefig(tracking_path, dpi=100, bbox_inches='tight')
    plt.close()

    # Matching
    fig2 = visualize_example(X2, y2, meta2, sample_idx=idx, task_type='matching')
    matching_path = IMAGES_DIR / f"matching_ex{idx}.png"
    plt.savefig(matching_path, dpi=100, bbox_inches='tight')
    plt.close()

    # QA
    fig3 = visualize_example(X3, y3, meta3, sample_idx=idx, task_type='babi')
    babi_path = IMAGES_DIR / f"babi_ex{idx}.png"
    plt.savefig(babi_path, dpi=100, bbox_inches='tight')
    plt.close()
